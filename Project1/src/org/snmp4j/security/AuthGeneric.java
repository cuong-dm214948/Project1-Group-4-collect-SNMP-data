...
public abstract class AuthGeneric implements AuthenticationProtocol {
    ...
    public byte[] changeDelta(byte[] oldKey,
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(protoName + "oldKey: " + PrivAES.asHex(oldKey));
            logger.debug(protoName + "newKey: " + PrivAES.asHex(newKey));
            logger.debug(protoName + "random: " + PrivAES.asHex(random));
        }
        int iterations = (oldKey.length - 1) / hash.getDigestLength();

        OctetString tmp = new SecretOctetString(oldKey);
        OctetString delta = new SecretOctetString();
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(protoName + "keyChange:" + new SecretOctetString(keyChange).toHexString());
        }
        return keyChange;
    }
    ...
    public byte[] passwordToKey(OctetString passwordString, byte[] engineID) {
        ...
        digest = md.digest();
        if (logger.isDebugEnabled()) {
            logger.debug(protoName + " first digest: " + new SecretOctetString(digest).toHexString());
        }
        ...
        digest = md.digest();
        if (logger.isDebugEnabled()) {
            logger.debug(protoName + " localized key: " + new SecretOctetString(digest).toHexString());
        }
