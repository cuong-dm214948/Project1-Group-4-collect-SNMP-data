...
/**
 * The {@code TableEvent} class reports events in a table retrieval
 * operation.
 *
 * @author Frank Fock
 * @version 1.8
...
public class TableEvent extends RetrievalEvent {
     ...
     * @param cols
     *    an array of {@code VariableBinding} instances containing the
     *    elements of the row. The array may contain {@code null} elements
     *    which indicates that the agent does not return an instance for that
     *    column and row index. If an element is not {@code null}, then
     *    the {@code OID} of the variable binding contains the full instance
     *    {@code OID} (i.e., table OID + column ID + row index) of the variable.
     */
     ...
     * @return
     *    the row's index OID or {@code null} if {@link #isError()} returns
     *    {@code true}.
     */
    public OID getIndex() {
        return index;
     ...
     *    column and row index. If an element is not {@code null}, then
     *    the {@code OID} of the variable binding contains the full instance
     *    {@code OID} of the variable.<p>
     *    If {@link #isError()} returns {@code true}, {@code null}
     *    will be returned.
     */
