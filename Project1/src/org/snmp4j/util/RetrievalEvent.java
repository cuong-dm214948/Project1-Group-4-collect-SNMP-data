...
/**
 * The {@code RetrievalEvent} is an abstract class representing the result
...
public abstract class RetrievalEvent extends EventObject {
  ...
  /**
   * The maximum number of to be retrieved has been reached. See {@link TableUtils#setRowLimit(int)}.
   */
  public static final int STATUS_ROW_LIMIT_REACHED = -5;
   ...
   *    failure of the operation which corresponds to an SNMP error status
   ...
   * @return
   *    {@code true} if the operation failed with an error.
   */
   ...
   *    {@link #STATUS_EXCEPTION}), or {@code null} otherwise.
   */
   ...
   * @return
   *    a {@code ScopedPDU} instance if there has been a report PDU
   *    instance associated with this event ({@link #getStatus()} returns
   *    {@link #STATUS_REPORT}), or {@code null} otherwise.
   */
