...
public class PDU implements BERSerializable, Serializable {
    ...
    /**
     * Gets all the {@link VariableBinding}s from this PDU whose object identifier ({@link OID}) equals or has the
     * same prefix than the specified object identifier or name.
     * @param objectIdentifierOrName
     *    a dotted numbered OID string like "1.3.6.1.4.1.4976" or an object name parsable by format as set by
     *    {@link SNMP4JSettings#setOIDTextFormat(OIDTextFormat)}. If such a OID format is not set before this
     *    method is called, or if the {@code objectIdentifierOrName} could not be parsed/does not comply to the
     *    format rules, then a {@link RuntimeException} will be thrown.
     * @return
     *    a list of {@link VariableBinding}s whose {@link OID} is a parent of the object identifier(s) given as
     *    argument. The list is ordered by the given prefix OIDs and within the matches of each prefix OID, the list
     *    is ordered by occurrence in this {@link PDU}.
     * @since 3.5.0
     */
    public List<VariableBinding> get(String ...objectIdentifierOrName) {
        ArrayList<VariableBinding> result = new ArrayList<>();
        for (String objectName : objectIdentifierOrName) {
            OID prefixOID;
            try {
                prefixOID = new OID(SNMP4JSettings.getOIDTextFormat().parse(objectName));
                for (VariableBinding vb : variableBindings) {
                    if (vb.getOid().startsWith(prefixOID)) {
                        result.add(vb);
                    }
                }
            } catch (ParseException e) {
                throw new RuntimeException("Given object name '"+objectName+
                        "' could not be parsed, is SNMP4J-SMI-PRO's SmiManager configured correctly with SNMP4JSettings.setOIDTextFormat(smiManager)?",
                        e);
            }
        }
        return result;
     ...
     *
     * @param prefix the search {@link OID}. If {@code null}, then all variable bindings will be returned.
    ...
    /**
     * Gets all the {@link VariableBinding}s of this PDU.
     * @return
     *    a possibly empty list of {@link VariableBinding} instances.
     * @since 3.5.0
     */
